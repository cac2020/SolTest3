...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  //使用ethers.js库获取名为"FeeManager"的智能合约工厂对象。`getContractFactory`是一个异步函数，用于从已编译的合约ABI和字节码创建合约工厂实例，以便后续部署或与该合约进行交互。
  const FeeManager = await ethers.getContractFactory("FeeManager");
  //通过代理部署合约
  const proxy = await upgrades.deployProxy(
    FeeManager,
    [deployer, 200, deployer],//费率2%
    {
...
module.exports.tags = ["deployFeeManager", "all"];
